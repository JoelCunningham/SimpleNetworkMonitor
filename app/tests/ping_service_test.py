...
def test_get_ttl_from_ping():
    ...
    ps = PingService()
    ttl = ps.get_ttl_from_ping(ping_out)
    assert ttl == 64


def test_get_os_from_ttl_exact():
    ps = PingService()
    assert ps.get_os_from_ttl(64) == "Linux/Unix/macOS"
    assert ps.get_os_from_ttl(128).startswith("Windows")


def test_get_os_from_ttl_via_router():
    ps = PingService()
    # TTL of 62 (within 60 +/-10) should map via router to Other Linux
    assert "via router" in ps.get_os_from_ttl(62)
